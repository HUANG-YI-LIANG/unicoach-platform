...
export function calculateBookingPrice({ 
  ...
  // Coach payout is calculated from BASE PRICE, meaning the platform subsidizes the discount.
  // Student penalty is split 50/50.
  const baseCoachPayout = roundMoney(normalizedBasePrice * (1 - effectiveCoachCommission / 100));
  const coachExtraFromStudentPenalty = roundMoney(studentPenaltyFee / 2);
  const coachPayout = baseCoachPayout + coachExtraFromStudentPenalty;

  // Calculate intended discount
  const totalDiscountPercent = clampPercent(normalizedBaseDiscountPercent + normalizedCouponDiscountPercent);
  let intendedDiscountAmount = roundMoney(Math.min(
    Math.round(normalizedBasePrice * (totalDiscountPercent / 100)),
    MAX_DISCOUNT_AMOUNT
  ));
  
  // Include the penalty discount for UI
  let totalDiscountForStudent = intendedDiscountAmount + coachPenaltyDiscount;
  
  // PROFIT GUARD: Ensure platform margin is at least 20% of finalPrice
  // finalPrice = basePrice - totalDiscountForStudent + studentPenaltyFee
  // platformFee = finalPrice - coachPayout >= 0.20 * finalPrice
  // => 0.80 * finalPrice >= coachPayout
  // => finalPrice >= coachPayout / 0.80
  // => basePrice - totalDiscountForStudent + studentPenaltyFee >= coachPayout / 0.80
  // => totalDiscountForStudent <= basePrice + studentPenaltyFee - Math.ceil(coachPayout / 0.80)
  
  const maxAllowedDiscount = Math.max(0, normalizedBasePrice + studentPenaltyFee - Math.ceil(coachPayout / 0.80));
  
  if (totalDiscountForStudent > maxAllowedDiscount) {
    totalDiscountForStudent = maxAllowedDiscount;
  }
  ...
  return {
    ...
    discountAmount: totalDiscountForStudent, 
    finalPrice,
    depositPaid,
    platformFee,
    coachPayout,
