...
export function calculateBookingPrice({ basePrice, baseDiscountPercent = 0, couponDiscountPercent = 0, coachCommission = 20 }) {
  ...
  const depositPaid = roundMoney(finalPrice * 0.3);
  const normalizedCommission = clampPercent(coachCommission, 20);
  const platformFee = roundMoney(normalizedBasePrice * (normalizedCommission / 100));
  const coachPayout = roundMoney(normalizedBasePrice - platformFee);
