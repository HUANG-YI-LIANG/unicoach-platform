...
export function calculateBookingPrice({ basePrice, attendeesCount = 1, baseDiscountPercent = 0, couponDiscountPercent = 0, coachCommission = 20 }) {
  const normalizedBasePrice = Math.max(0, Math.round(Number(basePrice) || 0));
  
  const count = Math.max(1, Number(attendeesCount) || 1);
  const effectiveCount = Math.min(count, 5); // 價格折讓以 5 個人為限
  const perPersonPrice = Math.max(0, normalizedBasePrice - 50 * (effectiveCount - 1));
  ...
  const discountAmount = Math.min(Math.round(totalBasePrice * (totalDiscountPercent / 100)), MAX_DISCOUNT_AMOUNT);
  ...
  const normalizedCommission = Number.isFinite(Number(coachCommission)) && Number(coachCommission) >= 0
    ...
    : 20;
  const platformFee = Math.round(totalBasePrice * (normalizedCommission / 100));
  const coachPayout = Math.max(0, totalBasePrice - platformFee);
