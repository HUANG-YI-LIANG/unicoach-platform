...
export function calculateBookingPrice({ basePrice, baseDiscountPercent = 0, couponDiscountPercent = 0, coachCommission = 20 }) {
  ...
  const finalPrice = roundMoney(normalizedBasePrice - discountAmount);
  const split = calculatePriceSplitFromFinalPrice({ finalPrice, coachCommission });

  return {
    totalDiscountPercent,
    discountAmount,
    finalPrice: split.finalPrice,
    depositPaid: split.depositPaid,
    platformFee: split.platformFee,
    coachPayout: split.coachPayout,
  };
}
