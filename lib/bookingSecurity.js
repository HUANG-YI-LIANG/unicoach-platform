...
export function getServerCouponDiscount({ requestedCouponId, claimedCoupons = [], now = new Date() }) {
  ...
  const percent = clampPercent(coupon.discount);

  return { couponId: coupon.id, percent, coupon };
}

export function calculatePriceSplitFromFinalPrice({ finalPrice, coachCommission = 20 }) {
  const normalizedFinalPrice = roundMoney(finalPrice);
  const normalizedCommission = clampPercent(coachCommission, 20);
  const depositPaid = roundMoney(normalizedFinalPrice * 0.3);
  const platformFee = roundMoney(normalizedFinalPrice * (normalizedCommission / 100));
  const coachPayout = roundMoney(normalizedFinalPrice - platformFee);
...
export function assertFinancialPriceInvariant({ finalPrice, platformFee, coachPayout }) {
  const normalizedFinalPrice = roundMoney(finalPrice);
  const normalizedPlatformFee = roundMoney(platformFee);
  const normalizedCoachPayout = roundMoney(coachPayout);

  if (normalizedCoachPayout > normalizedFinalPrice) {
    return { ok: false, error: '教練分潤不可高於訂單實收金額' };
  }

  if (normalizedPlatformFee > normalizedFinalPrice) {
    return { ok: false, error: '平台費不可高於訂單實收金額' };
  }

  if (normalizedPlatformFee + normalizedCoachPayout > normalizedFinalPrice) {
    return { ok: false, error: '平台費與教練分潤總和不可高於訂單實收金額' };
  }
...
export function calculateBookingPrice({ 
  ...
  const normalizedBasePrice = roundMoney(basePrice);
  const normalizedBaseDiscountPercent = clampPercent(baseDiscountPercent);
  const normalizedCouponDiscountPercent = clampPercent(couponDiscountPercent);
  
  // Penalties calculation
  const validStudentWarnings = Math.max(0, Number.isInteger(Number(studentWarnings)) ? Number(studentWarnings) : 0);
  const validCoachWarnings = Math.max(0, Number.isInteger(Number(coachWarnings)) ? Number(coachWarnings) : 0);
  
  const studentPenaltyFee = roundMoney(normalizedBasePrice * (validStudentWarnings * 0.05));
  const coachPenaltyDiscount = roundMoney(normalizedBasePrice * (validCoachWarnings * 0.025));

  const totalDiscountPercent = clampPercent(normalizedBaseDiscountPercent + normalizedCouponDiscountPercent);
  const discountAmount = roundMoney(Math.min(
    Math.round(normalizedBasePrice * (totalDiscountPercent / 100)),
    MAX_DISCOUNT_AMOUNT
  ));
  
  // Apply the coach penalty discount to the student
  const totalDiscountForStudent = discountAmount + coachPenaltyDiscount;
  
  // The final price the student pays
  const finalPrice = roundMoney(normalizedBasePrice - totalDiscountForStudent + studentPenaltyFee);

  // Coach penalty increases their commission, which decreases their payout
  // Effective coach commission = base + (coach_warnings * 5%)
  const effectiveCoachCommission = clampPercent(coachCommission + (validCoachWarnings * 5), 20);
  
  // Base payout split from the NET price (Base - Normal Discount)
  // We use this so the platform and coach share the burden of normal discounts, 
  // but penalties are calculated cleanly.
  const netPriceForSplit = Math.max(0, normalizedBasePrice - discountAmount);
  const depositPaid = roundMoney(finalPrice * 0.3); // Deposit is 30% of final price
  
  // Base platform fee based on effective commission
  const basePlatformFee = roundMoney(netPriceForSplit * (effectiveCoachCommission / 100));
  
  // Student penalty fee is split 50/50 between platform and coach
  const coachExtraFromStudentPenalty = roundMoney(studentPenaltyFee / 2);
  const platformExtraFromStudentPenalty = studentPenaltyFee - coachExtraFromStudentPenalty;

  // The coach's final payout
  const coachPayout = roundMoney(netPriceForSplit - basePlatformFee + coachExtraFromStudentPenalty);
  
  // The platform's final fee
  // Note: Platform fee absorbs the cost of the 2.5% discount given to the student from the coach's 5% penalty
  // because the coach was charged 5% (effectiveCoachCommission increased), but only 2.5% went to the student.
  const platformFee = roundMoney(finalPrice - coachPayout);

  return {
    totalDiscountPercent,
    discountAmount: totalDiscountForStudent, // Include the penalty discount for UI
    finalPrice,
    depositPaid,
    platformFee,
    coachPayout,
    studentPenaltyFee,
    coachPenaltyDiscount
  };
}
...
export function isBookingTimeAllowed({ expectedTime, durationMinutes, rules = [], exceptions = [], legacyAvailableTimes = null }) {
  ...
  for (let cursor = start; cursor < end; cursor += SLOT_MINUTES) {
    ...
    const chunkEnd = cursor + SLOT_MINUTES;

    const chunkAllowedByException = dateExceptions.some((exception) => (
      exception.type === 'available' && rangeContains(chunkStart, chunkEnd, exception.startMinutes, exception.endMinutes)
    ));
    if (chunkAllowedByException) continue;

    const chunkAllowedByRule = effectiveRules.some((rule) => (
      rule.weekday === parts.weekday &&
      chunkStart % rule.slotMinutes === 0 &&
      rangeContains(chunkStart, chunkEnd, rule.startMinutes, rule.endMinutes)
    ));
    if (chunkAllowedByRule) continue;
