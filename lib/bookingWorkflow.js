...
export function getPendingPaymentExpirationState(booking, now = new Date()) {
  if (!booking || booking.status !== 'pending_payment' || !booking.payment_expires_at) {
  ...
  if (Number.isFinite(expiresAt) && Number.isFinite(nowTime) && nowTime > expiresAt) {
    return {
      expired: true,
      status: 409,
...
export function buildExpiredPendingPaymentUpdate() {
  return {
    status: 'cancelled',
...
export function canCompleteBooking(booking, { hasFinalReport = false, now = new Date() } = {}) {
  if (!booking) return { ok: false, status: 404, error: '找不到該預約記錄' };

  if (booking.payment_status !== 'paid') {
    return { ok: false, status: 400, error: '預約尚未完成付款，不能標記為完成。' };
  }

  if (!hasFinalReport) {
    return { ok: false, status: 400, error: '必須先填寫正式學習報告，才能將課程標記為完成。' };
  }

  const expectedTime = new Date(booking.expected_time).getTime();
  const durationMinutes = Number(booking.duration_minutes) > 0 ? Number(booking.duration_minutes) : 60;
  const classEndTime = expectedTime + durationMinutes * 60 * 1000;
  const nowTime = now instanceof Date ? now.getTime() : new Date(now).getTime();

  if (!Number.isFinite(expectedTime) || !Number.isFinite(nowTime)) {
    return { ok: false, status: 400, error: '預約時間資料不合法，不能標記為完成。' };
  }

  if (nowTime < classEndTime) {
    return { ok: false, status: 400, error: '課程尚未結束，不能標記為完成。' };
  }
...
export function canTransitionBookingStatus({ actor, booking, newStatus, hasFinalReport = false }) {
  ...
  if (!VALID_BOOKING_STATUSES.has(newStatus)) {
    return { ok: false, status: 400, error: '不支援的預約狀態。', role };
  }
  ...
  if (newStatus === 'completed') {
    const completion = canCompleteBooking(booking, { hasFinalReport });
    if (!completion.ok) return { ...completion, role };
  }
