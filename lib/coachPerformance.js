...
export async function getCoachPerformanceByUserId(userId, supabaseAdmin) {
  ...
  try {
    ...
    try {
      ...
      // Pioneer coach check
      const { data: userData } = await supabase.from('users').select('created_at, role').eq('id', userId).single();
      if (userData && userData.role === 'coach') {
         ...
         if (count && count <= topSettings.top_n) {
            isPioneer = true;
            pioneerDiscount = Number(topSettings.bonus_discount) || 5;
         }
      }
    } catch (e) {
      console.warn('[Coach Performance] Error fetching discount/pioneer status', safeErrorDetails(e));
    }
    ...
    currentCommission = Math.max(0, baseCommission - personalDiscount - pioneerDiscount);

    // Apply manual temporary override if exists
    let finalLevel = currentLevel;
    try {
      // Import getAdminSupabase dynamically to avoid circular dependencies if any
      ...
      if (authData?.user?.user_metadata?.granted_level) {
        ...
        if (new Date(gl.expires_at) > new Date()) {
          finalLevel = Math.max(currentLevel, gl.level);
        }
      }
    } catch (e) {
      console.warn('[Coach Performance] Error fetching granted_level', e);
    }
...
/**
 * 取得教練的影片與照片上傳額度 (根據等級，並優先採用 user_metadata 客服設定)
 */
export async function getCoachMediaLimits(userId, supabaseAdmin) {
  const supabase = supabaseAdmin || await getDefaultAdminSupabase();
  let max_videos = 3; // Lv1 預設
  let max_photos = 5; // Lv1 預設

  try {
    // 1. 取得使用者的 user_metadata (檢查是否有客服特調)
    const { data: authUser, error: authError } = await supabase.auth.admin.getUserById(userId);
    if (authError) throw authError;

    const userMetadata = authUser?.user?.user_metadata || {};
    const hasCustomVideos = userMetadata.max_videos !== undefined && userMetadata.max_videos !== null;
    const hasCustomPhotos = userMetadata.max_photos !== undefined && userMetadata.max_photos !== null;

    // 2. 如果沒有特調，則根據等級給予
    if (!hasCustomVideos || !hasCustomPhotos) {
      const performance = await getCoachPerformanceByUserId(userId, supabase);
      const levelNum = performance.currentLevel || 1;
      ...
      const { data: settingsData } = await supabase
        ...
        .select('value')
        .eq('key', 'coach_tier_rates')
        .single();

      if (settingsData && settingsData.value) {
        try {
          const tiers = typeof settingsData.value === 'string' ? JSON.parse(settingsData.value) : settingsData.value;
          const tier = tiers.find(t => t.level === levelNum) || tiers[tiers.length - 1];
          if (tier) {
            if (tier.max_videos !== undefined) tierMaxVideos = tier.max_videos;
            if (tier.max_photos !== undefined) tierMaxPhotos = tier.max_photos;
          }
        } catch (e) {
          console.error('Failed to parse coach_tier_rates', e);
        }
      }

      if (!hasCustomVideos) max_videos = tierMaxVideos;
      if (!hasCustomPhotos) max_photos = tierMaxPhotos;
    }

    if (hasCustomVideos) max_videos = Number(userMetadata.max_videos);
    if (hasCustomPhotos) max_photos = Number(userMetadata.max_photos);

  } catch (err) {
    console.error('[Get Coach Media Limits Error]', safeErrorDetails(err));
  }

  return { max_videos, max_photos };
}
