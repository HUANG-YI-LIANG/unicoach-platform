...
export async function getCoachPerformance(coachId, supabaseAdmin) {
  ...
  try {
    ...
    // Fetch coach personal discount
    const { data: coachData } = await supabase.from('coaches').select('commission_discount').eq('id', coachId).maybeSingle();
    const personalDiscount = coachData?.commission_discount || 0;
